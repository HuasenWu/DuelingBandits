...
void CAlg_RMED::init(int num_slots, int num_arms, struConfig config) {
...
int CAlg_RMED::pull(int ind_slot, vector<double> reward, vector<vector<int>> beat, vector<int>& cmpPair) {
	...
	// Second stage for RMED
	else if (indAlg == ALG_RMED2 && ind_slot < num_pairs * scale_factor * log(log(ind_slot + 1))) {
	...
	// Remaining slots
		...
		// find the arm with minimum divergence
		int ind_min_div = -1;
		double val_min_div = minEmpiDiv(ind_min_div);
		...
		ind_second = -1;
		// RMED2: choose \hat{b}^* if it is in the candidate set 
		if (indAlg == ALG_RMED2) {
			// Find \hat{b}^*
			int ind_min_ratio;
			minRatio(ind_first, ind_min_div, ind_min_ratio);
...
void CAlg_RMED::calEmpiBeatProb() {
...
double CAlg_RMED::minEmpiDiv(int& ind_min_div) {
...
double CAlg_RMED::minRatio(int ind_first, int ind_min_div, int& ind_min_ratio) {
