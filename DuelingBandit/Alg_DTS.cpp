...
void CAlg_DTS::init(int num_slots, int num_arms, struConfig config) {
...
int CAlg_DTS::pull(int ind_slot, vector<double> reward, vector<vector<int>> beat, vector<int>& cmpPair) {
	cmpPair.clear();
	...
	if (candidates.empty()) {
		...
		ind_first = argmaxGivenIndSet(counts1, candidates);		
	}
...
void CAlg_DTS::calRCB(int ind_slot) {
	for (int ind_arm1 = 0; ind_arm1 < num_arms; ind_arm1++) {
		for (int ind_arm2 = 0; ind_arm2 <= ind_arm1; ind_arm2++) {
			if (ind_arm2 != ind_arm1) {
...
void CAlg_DTS::calKLRCB(int ind_slot) {
...
void CAlg_DTS::genSamples(int ind_col) {
	double gamma_scale = 2.0;
